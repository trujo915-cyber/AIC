"""
Gestor de "perfiles de calificación": el material de referencia (rúbrica,
guía de prácticas, formato de ejemplo -- en texto y/o archivos) más los
ejemplo(s) ya calificados que Carla arma una vez por práctica y reutiliza
cada vez que vuelve a calificar esa misma práctica.
...
Los documentos (ejemplos y material de referencia) se guardan codificados
en base64 dentro del mismo JSON, así todo queda en un solo archivo portátil.
"""
...
from .schemas import ArchivoReferencia, EjemploCalificado


@dataclass
class Perfil:
    id: str  # identificador simple, ej. "TC1-Ley Stephan Boltzmann|coloquio"
    materia: str
    practica: str
    tipo: str  # "coloquio" | "informe"
    rubrica_texto: str  # texto libre: rúbrica, guía, notas -- lo que sea
    puntaje_maximo: float
    ejemplos: list[EjemploCalificado] = field(default_factory=list)
    material_archivos: list[ArchivoReferencia] = field(default_factory=list)
...
def _perfil_a_dict(p: Perfil) -> dict:
    ...
    for arch in d["material_archivos"]:
        arch["data"] = base64.b64encode(arch["data"]).decode("ascii")
    return d
...
def _dict_a_perfil(d: dict) -> Perfil:
    ...
    material_archivos = [
        ...
        for a in d.get("material_archivos", [])
    ]
    return Perfil(
        id=d["id"],
        materia=d["materia"],
...
def exportar(perfiles: dict[str, Perfil]) -> bytes:
    ...
    data = {"version": 2, "perfiles": [_perfil_a_dict(p) for p in perfiles.values()]}
