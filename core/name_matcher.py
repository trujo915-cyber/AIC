...
from .text_utils import normalizar, tokens
...
def emparejar(
    ...
    """
    det_norm = normalizar(nombre_detectado)
    det_tokens = tokens(nombre_detectado)

    resultados = []
    for est in candidatos:
        cand_norm = normalizar(est.nombre)
        cand_tokens = tokens(est.nombre)
