...
MAX_CARACTERES_COMENTARIO = 1200  # Excel no limita mucho, pero un comentario gigante es incómodo de leer


def formatear_comentario(resultado: ResultadoCalificacion) -> str:
    """Arma el mismo desglose que se anota en el PDF, en texto plano, para
    dejarlo como comentario de celda -- así se ve el detalle por sección sin
    salir del Excel mientras se revisan las notas."""
    lineas = ["Calificación con apoyo de IA:"]
    for item in resultado.desglose:
        lineas.append(f"- {item.criterio}: {item.puntaje_obtenido:g}/{item.puntaje_maximo:g} — {item.comentario}")
    lineas.append(f"\nTotal sugerido: {resultado.nota_sugerida:g}")
    if resultado.observaciones:
        lineas.append(f"\nComentario general: {resultado.observaciones}")
    texto = "\n".join(lineas)
    if len(texto) > MAX_CARACTERES_COMENTARIO:
        texto = texto[: MAX_CARACTERES_COMENTARIO - 1].rstrip() + "…"
    return texto
...
def escribir_nota(
    ...
    Si se pasa `comentario`, queda como nota de celda (visible al pasar el
    cursor), con el desglose de la calificación."""
    ...
    celda.value = valor
    if comentario:
        celda.comment = Comment(comentario, "Calificación IA", width=340, height=220)
