...
from core.excel_writer import escribir_nota, formatear_comentario
...
if hoja is not None:
    ...
    if st.button("✨ Calificar con IA", type="primary", disabled=not docs_subidos):
        if not api_key:
            ...
            texto_espera = f"Calificando {len(documentos)} documento(s)..."
            if len(documentos) > 5:
                texto_espera += " Con varios documentos esto va con pausas cortas a propósito (para no toparse el límite de la API) -- puede tardar unos minutos, es normal."
            with st.spinner(texto_espera):
                try:
                    items = procesar_lote(
    ...
    if st.button("✍️ Aplicar sobrescrituras seleccionadas"):
        ...
        for i, c in enumerate(st.session_state.conflictos_pendientes):
            if bool(df_conf.iloc[i]["Sobrescribir"]) and ws is not None:
                r = escribir_nota(
                    ws, hoja, c["practica"], c["estudiante"], c["campo"], c["valor_nuevo"],
                    forzar=True, comentario=c.get("comentario"),
                )
                if r.ok:
...
if items:
    ...
    if st.button("✅ Aplicar notas confirmadas al Excel", type="primary"):
        ...
        for i_fila, fila in df_editado.iterrows():
            ...
            comentario_celda = formatear_comentario(items[i_fila].resultado_ia)
            for est in destinatarios:
                valor_nuevo = float(fila["Nota sugerida"])
                r = escribir_nota(ws, hoja, practica_rev, est, campo, valor_nuevo, comentario=comentario_celda)
                if r.ok:
                    aplicados += 1
